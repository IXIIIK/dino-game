...
import sprites from '/static/lib/sprites.js'
...
function sendDataToFlask(valueFromJS) {
  var data = {
      key1: valueFromJS,
      // Другие данные
  };
  ...
  $.ajax({
    type: 'POST',
    data: data,  // Отправка данных как формы
    url: '/game',
    success: function(response) {
        console.log(response);
        // Обработка ответа от Flask-приложения
    },
...
export default class DinoGame extends GameRunner {
  ...
  async preload() {
    ...
    const [spriteImage] = await Promise.all([
      loadImage('static/assets/sprite.png'),
      loadFont('static/assets/PressStart2P-Regular.ttf', 'PressStart2P'),
    ])
  ...
  onFrame() {
    ...
    if (state.isRunning) {
      ...
      if (state.gameOver) {
        this.endGame()
        sendDataToFlask(state.score.value);
